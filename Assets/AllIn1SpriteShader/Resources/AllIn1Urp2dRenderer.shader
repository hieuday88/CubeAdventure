Shader "AllIn1SpriteShader/AllIn1Urp2dRenderer"
{
    Properties
    {
		...
		[Space]
		[Toggle()] _DissolveEnabled("Enable Disintegration", Float) = 0
		_DissolveFlowMap("Dissolve Flow (RG)", 2D) = "black" {}
		_DissolveTex("Dissolve Texture", 2D) = "white" {}
		_DissolveColor("Dissolve Border Color", Color) = (1, 1, 1, 1)
		_DissolveBorder("Dissolve Border", Range(0.001, 0.25)) = 0.05
		_DissolveExpand("Dissolve Expand", Float) = 1
		_DissolveWeight("Dissolve Weight", Range(0,1)) = 0
		_DissolveDirection("Dissolve Direction", Vector) = (0, 0, 0, 0)
		[HDR] _DisintegrationColor("Disintegration Color", Color) = (1, 1, 1, 1)
		_DissolveGlow("Dissolve Glow", Float) = 1
		_DissolveShapeTex("Dissolve Shape", 2D) = "white" {}
		_DissolveRadius("Dissolve Shape Radius", Range(0,1)) = 0.5
        ...
        Pass
        {
			...
			sampler2D _DissolveFlowMap, _DissolveTex, _DissolveShapeTex;
			half4 _DissolveFlowMap_ST;
			half4 _DissolveColor, _DissolveDirection, _DisintegrationColor;
			half _DissolveBorder, _DissolveExpand, _DissolveWeight, _DissolveGlow, _DissolveRadius, _DissolveEnabled;
            ...
            v2f vert (appdata v)
            {
				...
				o.flowUV = TRANSFORM_TEX(v.uv, _DissolveFlowMap);
				o.color = v.color;
            ...
            half4 frag (v2f i) : SV_Target
            {
				...
				* max(sign((_ShineLocation + _ShineWidth) - currentDistanceProjection), 0.0) * _ShineColor * shineMask;
				...
				#if DISSOLVE_ON
				if (_DissolveEnabled > 0.5 || _DissolveWeight > 0.0001)
				{
					float2 flow = RemapFlowRG(tex2D(_DissolveFlowMap, i.flowUV).rg);
					float2 dissolveUV = uvRect + (_DissolveDirection.xy + flow * _DissolveExpand) * _DissolveWeight;

					if (_DissolveWeight >= 0.9999)
					{
						clip(-1.0);
					}

					float dissolve = tex2D(_DissolveTex, dissolveUV).r;
					float threshold = saturate(_DissolveWeight * 2.0);
					...
					float shapeMask = step(saturate(_DissolveRadius), tex2D(_DissolveShapeTex, uvRect).r);

					half3 edgeColor = _DissolveColor.rgb * (_DissolveGlow * edge);
					half3 disintegration = _DisintegrationColor.rgb * (_DissolveGlow * edge * shapeMask);
					col.rgb += edgeColor + disintegration;
				}
				#endif

				SurfaceData2D surfaceData;
				InputData2D inputData;
				InitializeSurfaceData(col.rgb, col.a, half4(1, 1, 1, 1), surfaceData);
