...
public class AllIn1SpriteShaderMaterialInspector : ShaderGUI
{
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        if(currEnabledDrawers[uvFxShapeDrawer])
        {
            ...
            DissolveIntegration("Disintegration");
        }
    ...
    private void DissolveIntegration(string inspector)
    {
        MaterialProperty dissolveEnabled = ShaderGUI.FindProperty("_DissolveEnabled", matProperties, false);
        if(dissolveEnabled == null) return;

        bool ini = dissolveEnabled.floatValue > 0.5f;
        bool toggle = ini;

        GUIContent effectNameLabel = new GUIContent();
        effectNameLabel.tooltip = "_DissolveEnabled (C#)";
        effectNameLabel.text = effectCount + "." + inspector;
        toggle = EditorGUILayout.BeginToggleGroup(effectNameLabel, toggle);
        ...
        if(ini != toggle)
        {
            dissolveEnabled.floatValue = toggle ? 1f : 0f;
            Save();
        }
        ...
        if(toggle)
        {
            EditorGUILayout.BeginVertical(propertiesStyle);
            {
                DrawProperty("_DissolveFlowMap");
                DrawProperty("_DissolveTex");
                DrawProperty("_DissolveShapeTex");
                DrawProperty("_DissolveDirection");
                DrawProperty("_DissolveWeight");
                DrawProperty("_DissolveExpand");
                DrawProperty("_DissolveBorder");
                DrawProperty("_DissolveGlow");
                DrawProperty("_DissolveRadius");
                DrawProperty("_DissolveColor");
                DrawProperty("_DisintegrationColor");
            }
            EditorGUILayout.EndVertical();
        }
    ...
    private void DrawProperty(string propertyName, bool noReset = false)
    {
        MaterialProperty targetProperty = ShaderGUI.FindProperty(propertyName, matProperties, false);
